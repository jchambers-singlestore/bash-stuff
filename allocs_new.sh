...
total_server_memory=$(grep -i total_server_memory $filename | sed 's/^[0-9]* [0-9]*-[0-9]*-[0-9]* [0-9]*:[0-9]*:[0-9]*\.[0-9]*  ERROR://' | sed 's/.*|.*|.*|.*|\([^|]*\) | \([^|]*\).*/\1 | \2/g' | sed 's/|//g')
...
for key in "${mem_keys[@]}"
do
  mem=$(grep -i "$key" $filename | sed 's/^[0-9]* [0-9]*-[0-9]*-[0-9]* [0-9]*:[0-9]*:[0-9]*\.[0-9]*  ERROR: //' | sed 's/.*|.*|.*|.*|\([^|]*\) | \([^|]*\).*/\1 | \2/g' | sed 's/|//g')
  if [ -z "$mem" ]; then
    echo "  ├─  $key (not found)"
  else
      ...
      do
        ...
        if [ -z "$sub_mem" ]; then
          ...
          if [ "$sub_key" == "Alloc_table_memory" ]; then
            ...
            do
              sub_sub_mem=$(grep -i "$sub_sub_key" $filename | rg -v 'variable_[abc]' | rg -v rust | sed 's/^[0-9]* [0-9]*-[0-9]*-[0-9]* [0-9]*:[0-9]*:[0-9]*\.[0-9]*  ERROR: //' | sed 's/.*|.*|.*|.*|\([^|]*\) | \([^|]*\).*/\1 | \2/g' | sed 's/|//g')
              if [ -z "$sub_sub_mem" ]; then
                echo "  │    │   ├─ $sub_sub_key (not found)"
              else
                if [ "$sub_sub_key" == "Alloc_hash_buckets" ] || [ "$sub_sub_key" == "Alloc_large_variable" ]; then
                  sub_sub_mem+=" * NOT included in Buffer_manager_memory"
                  sub_sub_mem=$(echo "$sub_sub_mem" | sed 's/MB[ \t]*\*/MB */g')
                  echo "  │       ├─ $sub_sub_mem"
                else
                  echo "  │       ├─ $sub_sub_mem"
                fi
