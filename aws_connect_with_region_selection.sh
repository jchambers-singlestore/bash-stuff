...
echo
read -p "Do you wish to use $region? (y/n) " -r
echo

if [[ $REPLY =~ ^[Yy]$ ]]
then
  echo "Proceeding with $region..."
  echo
else
  #List all available AWS regions
  echo "Available AWS regions:"
  aws ec2 describe-regions --output text | cut -f4
  read -p "Enter the desired AWS region: " desired_region
  aws configure set region $desired_region
  echo "Proceeding with region $desired_region"
fi


# Check to see if any instances with the specified tag are found in the region
if [ -z "$(aws ec2 describe-instances --filters Name=tag:Name,Values="*$tag*" \
--query 'Reservations[*].Instances[*].[InstanceId, PrivateIpAddress, PublicIpAddress, Tags[?Key==`Name`].Value[]]' | awk 'NF>1')" ]
then
    echo "No AWS instances found in this region with the tag specified. Try again and ensure the region has an instance with the specified tag."
    exit 1
fi
...
function ssh_instance {
    ...
    do
        echo "Waiting for instance to start...trying again in 1 second..."
        sleep 1
        instance_status=$(aws ec2 describe-instances --instance-ids $instance_id --query "Reservations[*].Instances[*].State.Name" --output text)
    done
